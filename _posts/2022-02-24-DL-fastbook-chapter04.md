...
<br>Note that when grad is used the gradient function calculated by PyTorch will be exhausted and reusing it will raise an error. In prior to reusing grad the the function f(xt) must be re-calculated, i.e., (yt = f(xt))." %}<br>
