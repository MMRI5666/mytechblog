...
<strong>Conclusion</strong>: The values calculated for the <strong>L1 norm (MEA)</strong> and the <strong>L2 norm (RSME)</strong> show that the distance between the selected 3 and the "ideal 3" is less than the distance to the ideal 7. Hence, we can conclude that this simple classifier model can give us the right prediction in this case.
