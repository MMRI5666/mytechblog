...
<em>Lists</em> are the most important data structure in <em>Lists</em> with the following characteristics:
  ...
  2.  Lists are first class citizens, which in functional programming concepts means that list can be passed to a function as argment, can be modified and assigned to a variable, and finally be returened to caller as the function output.
  3.   
